...
const emits = defineEmits<{
  sortByPriceAsc: [value: string]
  sortByPriceDesc: [value: string]
}>()
...
function sortByPrice(order: 'asc' | 'desc') {
  ...
  if (order === 'asc') {
    emits('sortByPriceAsc', '')
  }
  else {
    emits('sortByPriceDesc', '')
  }
}
</script>
      ...
      <div class="py-1">
        <MenuItem>
          <a
            ...
            :class="[activeSort === 'asc' ? 'bg-gray-200 text-gray-900' : 'text-gray-700']" @click="sortByPrice('asc')"
          >
            Цена по убыванию
          </a>
        </MenuItem>
        <MenuItem>
          <a
            href="#" class="block px-4 py-2 text-sm"
            :class="[activeSort === 'desc' ? 'bg-gray-200 text-gray-900' : 'text-gray-700']" @click="sortByPrice('desc')"
          >

...
const emits = defineEmits<{
  'form-submitted': [value: undefined]
}>()
...
<template>
  ...
  <vee-form @submit="submitCart" :validation-schema="cartSchema">
    <div class="mb-5 mt-5">
      <div class="text-lg font-medium text-gray-900 mb-6">Контактная информация</div>
      <div class="lg:w-5/12">
        <div class="mb-3 grid grid-cols-2 gap-2">
          <div>
            <label class="sr-only" for="name">Имя</label>
            <vee-field name="name" class="w-full rounded-lg border p-3 text-sm" placeholder="Имя" type="text"
                       id="name"/>
            <vee-error-message name="name" class="text-red-600 text-sm"/>
          </div>
          <div>
            <label class="sr-only" for="lastname">Фамилия</label>
            <vee-field name="lastname" class="w-full rounded-lg border p-3 text-sm" placeholder="Фамилия" type="text"
                       id="lastname"/>
        ...
        <div>
          <div class="flex flex-col">
            ...
            <vee-field name="email" class="w-6/12 rounded-lg border p-3 text-sm" placeholder="Почта" type="text"
                       id="email"/>
    ...
    <div class="text-lg font-medium text-gray-900 mb-6 mt-6">Данные для оплаты</div>
    <div class="lg:w-5/12">
      <div class="mb-3 grid grid-cols-2 gap-2">
        <div class="mb-3">
          <label class="sr-only" for="card-number">Номер карты</label>
          <vee-field name="card-number" class="w-full rounded-lg border p-3 text-sm" placeholder="Номер карты"
        ...
        <div class="mb-3">
          ...
          <vee-field name="card-name" class="w-full rounded-lg border p-3 text-sm" placeholder="Имя карты" type="text"
                     id="card-name"/>
    ...
    <div class="lg:w-5/12">
      <div class="grid grid-cols-2 lg:w-4/12 gap-2 mb-6">
        <div class="mb-3">
          ...
          <vee-field name="card-end-date" class="w-full rounded-lg border p-3 text-sm" placeholder="Срок" type="text"
                     id="card-end-date"/>
          <vee-error-message name="card-end-date" class="text-red-600 text-sm"/>
        </div>
        <div class="mb-3">
          <label class="sr-only" for="cvc">CVC</label>
          <vee-field name="cvc" class="w-full rounded-lg border p-3 text-sm" placeholder="CVC" type="text" id="cvc"/>
